...
class home extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      mebu_active:sessionStorage.mebu_active ? sessionStorage.mebu_active : '/Home/index',
      collapsed: false,
      menu_width:200,
      transition:0
    };
  ...
  render() {
    return (
      <div className="home">
        ...
        <div className="left_meun" style={{width:this.state.menu_width+'px',transition:this.state.transition+'s'}}>
          <div className="menu_logo items-center justify-center" style={{width:this.state.menu_width+'px',transition:this.state.transition+'s'}}>
          <img
          ...
          >
            {
                this.state.mebu_list.map((item, index) => {
                  if(item.name!=='系统设置'&& item.name!=='CRM管理'){
                    return <Menu.Item key={item.to} >{item.name}</Menu.Item>
                  }else{
                    return <SubMenu key={index} title={item.name}>
                      {
                        item.child.map((child)=>{
                          return <Menu.Item index={child.to} key={child.to} >{child.name}</Menu.Item>
                        })
                      }
                      </SubMenu>
                  }
        ...
        <div className="right_text">
          ...
          <div className="content">
            <Suspense fallback={<div className="justify-center items-center" style={{height:"calc(100% - 70px)"}}><Spin /></div>}>
              <Switch>
